...
def scan_unit(unit: dict[str, Any], api_key: str, model: str) -> ScanResult:
    ...
    enrichment = gemini_summary(api_key, model, unit, pages) if api_key and model else {}
    record = {
...
def main() -> None:
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=max(1, min(args.workers, 8))) as pool:
        futures = [pool.submit(scan_unit, unit, api_key, model) for unit in batch]
