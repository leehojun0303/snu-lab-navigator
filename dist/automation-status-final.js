  ...
  async function sync(){
    ...
    try{
      ...
      if(p.status==='idle'){
        ...
        if(status)status.textContent=limited?`Gemini 한도 도달 · 다음 5시간 주기에 재개 · 이번 실행 ${checked.toLocaleString()}개 처리`:(paused?`이번 실행 ${checked.toLocaleString()}개 처리 완료 · 다음 5시간 주기 대기`:(hasCounter?`이번 실행 · ${checked.toLocaleString()} / ${total.toLocaleString()}개 처리`:'상세정보 수집을 시작했습니다 · 첫 처리 묶음 저장 중'));
